...
double TableOfSatur[]={0.2,0.3,0.4,0.6,0.8,0.95};
int Nsatur=6;
...
int main(void)
{
 ...
 vector <int> path;  //hamilton cycle and euler cycle
	   ...
	   ofstream val_file2;
	   val_file2.open("Results_Hamilton_Only_new2cd.txt");
	   int TOCounts;
	    srand(time(NULL));
 for(int i=FROM;i<=TO;i+=STEP)
	 ...
	 for (int isatur=0;isatur<Nsatur;isatur++)
	  ...
	  // for(int ii=0;ii<=100;ii++){
	   	hamiltonCycle2(NLtTab,n,path,start,300000,notTimeOut);
	//	}
	   	stop=get_cpu_time();
	   	if(!notTimeOut)
		...
		cout<<"Hamilton cycle ready"<<endl;
		//*********************************************************	
		cout<<"Graph. n="<<n<<" saturation="<<saturation<<endl;
	   NLtTab=GenGraph3(n, saturation, edges);
       cout<<"Graph ready"<<endl;
