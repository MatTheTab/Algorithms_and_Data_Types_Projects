...
void generate_n_items(knapsack_item*, int);
...
void generate_worst_greedy(knapsack_item *tableOfItems, int nItems, int weight1, int val1, int weight2, int val2);
...
int main(void){
  ...
  for(int n=4;n<=N_CAP;n+=1){
	...
	generate_worst_greedy(arr_save, n, 11, 23, 5, 10);
   ofstream out;
   char bufor[100];
   sprintf(bufor,"Nknapsack2%06d.txt",n);
   out.open(bufor);
   for (int i=0;i<n;i++){
...
ofstream val_file;
val_file.open("Results_small_n_worst_greedy");
   ...
   for (int i=0;i<n;i++)
	...
	val_file.close();          
   	ofstream val_file2;
	val_file2.open("Results_big_n_worst_greedy.txt");
...
void generate_n_items(knapsack_item* items, int n){
	for(int i=0;i<n;i++){
...
void generate_worst_greedy(knapsack_item *tableOfItems, int nItems, int weight1, int val1, int weight2, int val2){
	tableOfItems[0].value=val1;
	tableOfItems[0].weight=weight1;
	tableOfItems[0].value_density=(double)val1/(double)weight1;
	for(int i=1;i<nItems;i++){
		tableOfItems[i].value=val2;
		tableOfItems[i].weight=weight2;
		tableOfItems[1].value_density=(double)val2/(double)weight2;
